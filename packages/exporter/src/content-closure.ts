...
export interface ContentClosureM3 {
  ...
  /** The reachable asset artifacts (`content/sha256/<digest>`), sorted by path. */
  assetArtifacts: readonly ClosureArtifact[];
  /** The reachable behavior artifacts (always empty — see `behaviors_unsupported`). */
  behaviorArtifacts: readonly ClosureArtifact[];
  behaviors: readonly ClosureBehavior[];
  /** Every manifest-declared artifact path, sorted and deduplicated. */
  declaredPaths: readonly string[];
 ...
 *
 * **Fail-closed on source-bearing behaviors (CC-55-3, open at Gate N):** the
 * M3 shared composition (`game-host`) has no behavior-linking channel yet, so a
 * captured v3 scene that names a source-bearing behavior is refused
 * (`export_build_unavailable` / `behaviors_unsupported`) rather than producing
 * a partial artifact whose behaviors could not run. The sample game (packet 61)
 * uses built-in modules only, so this does not block the M3 acceptance.
 *
...
export async function buildContentClosureM3(input: ContentClosureM3Input): Promise<{ ok: true; closure: ContentClosureM3 } | { ok: false; error: ContentClosureError }> {
  ...
  // 3. Source-bearing behaviors are refused (CC-55-3, fail-closed).
  const behaviorQuery = service.query({ op: 'queryBehaviors', projectId, args: { includeDeclaration: false, limit: 128, offset: 0 } });
  if (!behaviorQuery.ok) return { ok: false, error: fromCommandError(behaviorQuery.error) };
  const behaviorRows = (behaviorQuery as unknown as { behaviors: Array<Record<string, unknown>> }).behaviors;
  const hasSourceBehavior = behaviorRows.some((row) => {
    const source = row['source'];
    return source !== null && source !== undefined;
  });
  if (hasSourceBehavior) {
    return {
      ok: false,
      error: {
        code: 'export_build_unavailable',
        cls: 'unavailable',
        reason: 'behaviors_unsupported',
        message: 'an M3 export cannot carry source-bearing behaviors (the shared composition has no behavior-linking channel — CC-55-3)',
      },
    };
  }
  ...
  assetArtifacts.sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
  const declaredPaths = [...new Set(assetArtifacts.map((a) => a.path))].sort();
  return {
    ok: true,
    closure: {
