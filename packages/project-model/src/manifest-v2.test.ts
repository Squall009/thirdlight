...
describe('manifest-v2: captureManifestV2 assembly', () => {
  it('emits the v2 document in MANIFEST_KEYS_V2 order with buildId last', () => {
    ...
    expect(keys).toEqual(MANIFEST_KEYS_V2.filter((k) => k !== 'tags' && k !== 'materials' && k !== 'environment' && k !== 'scenes' && k !== 'buffers'));
    ...
    if (tagged.ok) {
      expect(Object.keys(tagged.manifest)).toEqual(MANIFEST_KEYS_V2.filter((k) => k !== 'materials' && k !== 'environment' && k !== 'scenes' && k !== 'buffers'));
...
describe('manifest-v2: contract constants', () => {
  it('the v2 key order carries the six added keys and buildId last', () => {
    expect(MANIFEST_KEYS_V2).toHaveLength(27); // incl. the optional phase-12 tags, scenes, buffers and the phase-9.4 materials, environment
