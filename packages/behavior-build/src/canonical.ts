 ...
 *
 * Two canonical forms are needed and must not be conflated:
...
/** UTF-8 encode without a BOM (the canonical bytes are BOM-free). */
export function utf8Encode(text: string): Uint8Array {
  return new TextEncoder().encode(text);
}

/** UTF-8 decode; the caller supplies bytes already accepted as valid UTF-8. */
...
/** Codepoint-order string comparison (not UTF-16 code-unit order). */
...
/**
 * Compact canonical JSON (keys sorted in codepoint order at every level,
 * shortest escapes, JS number semantics) — the digest form for the manifest
 * and the compile recipe (`manifestDigest`, recipe digest).
 */
export function canonicalJsonText(value: unknown): string {
  ...
  if (t === 'number') {
    ...
    if (!Number.isFinite(n)) throw new Error('canonicalJsonText: non-finite number');
    return JSON.stringify(n === 0 ? 0 : n);
  }
  if (t === 'boolean') return value === true ? 'true' : 'false';
  if (t === 'string') return JSON.stringify(value);
  if (Array.isArray(value)) return `[${value.map((v) => canonicalJsonText(v)).join(',')}]`;
  if (t === 'object') {
  ...
  throw new Error(`canonicalJsonText: unsupported value type ${t}`);
}
