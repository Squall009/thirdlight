 ...
 * deleteEntity inverses), redo (recorded entity, original ID, end of
 * array), redo invalidation by a fresh edit, and undo with swapped
 * previous/next + all-three changedFields.
 */
...
function initial(): CommandState<SceneV4> {
...
describe('scenario 05 — undo/redo with mixed human/agent edits (pure replay)', () => {
  ...
  it('replays all nine steps; every result payload matches the fixture verbatim', () => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      expect(msg.out['sceneId']).toBe('scene-main');
      expect(
        outcome.result,
        `step ${i + 1} (${JSON.stringify(msg.in).slice(0, 120)}…) result mismatch`,
      ).toEqual(withoutSceneId(msg.out));
      // The state carried into the next step is the new state.
      state = outcome.state;
    }

    // Final scene must equal the disk-after scene file's scene (canonical
    // bytes, revision 9, entities [cam-main, light-0001, light-0002,
    // box-0001, box-0002]).
