...
import type { ContentCatalog, Manifest, PrefabDefinition, SceneV2 } from '@thirdlight/project-model';
...
const AFTER = m2FixtureJson<{ scene: SceneV2; content: ContentCatalog; projectId: string; revision: number }>(
  'contracts/commands/prefab-scenario.after.json',
);
...
function stateThrough(id: string): CommandState<SceneV2> {
  let state = baseState();
  for (const s of STEPS) {
    ...
    if ((s.out as { duplicated?: boolean }).duplicated !== true) {
      ...
      state = r.state as CommandState<SceneV2>;
    }
...
const SNAPSHOT_BUILDERS: Record<string, () => CommandState<SceneV2>> = {
  base: baseState,
...
describe('deterministic ID allocation and limits (§8.7.2/§20.3)', () => {
  it('re-uses deleted IDs exactly as the accepted createEntity rule does', () => {
    ...
    const state = createCommandState(AFTER.scene, AFTER.content, MANIFEST);
  ...
  it('the result entity bound is checked before ID allocation (§8.7.5 step 6 order)', () => {
    // The 1024-entity model cap makes full group-prefix exhaustion unreachable
    // through a valid pipeline: the entity bound fires first (C10's id_exhaustion
    // remains a contract-required defensive branch, unit-tested above).
    ...
    for (let n = 1; n <= 9999; n++) {
      ...
      } as SceneV2['entities'][number]);
    }
    const state = createCommandState({ ...base.scene, entities } as SceneV2, base.content, MANIFEST);
    const before = JSON.stringify(state);
  ...
  it('rejects 65 overrides and a 257-entity definition with the contract limits', () => {
    ...
    const full = createCommandState(BEFORE.scene, { ...BEFORE.content, prefabs }, MANIFEST);
  ...
  it('rejects an instantiation that would exceed the scene entity or depth limit', () => {
    const state = stateThrough('M5');
    const definition = state.content?.prefabs[0] as PrefabDefinition;
    // Entity bound: pad the scene to 1024 entities, then instantiate.
    ...
    const full = createCommandState({ ...state.scene, entities } as SceneV2, state.content, MANIFEST);
    ...
    const deepState = createCommandState(
      { ...BEFORE.scene, entities: [...BEFORE.scene.entities, ...chain] } as SceneV2,
      state.content,
      MANIFEST,
    );
...
describe('fixtures/m2/prefabs/independence.messages.json replay', () => {
  it('replays every step byte-for-byte and ends at the indexed revision', () => {
    ...
    let state = createCommandState(AFTER.scene, AFTER.content, MANIFEST);
