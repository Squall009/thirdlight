**PROPOSED — not accepted.** Packet 40 (`docs/planning/m3-packets.md` §40). This
document proposes the normative text for a **new accepted contract home**
`docs/contracts/gameplay.md` plus the section-level edits to
`docs/contracts/runtime.md` listed in [`diffs/runtime.md`](diffs/runtime.md).
Nothing in `docs/contracts/**` is changed; Gate K records accept/reject per diff
and a separate docs-only promotion applies accepted rows before packet 44.
Packets 49/50/51 implement it; none of it is implemented here.

Read with:

- [`model.md`](model.md) §23.3.1/§23.3.2/§23.3.3/§23.4 (packet 39 **PROPOSED**
  v3 data: `gameZone`, `playerSpawn`, `cameraFollow`, `content.game`) — this
  document **consumes** that data and redefines none of it;
- [`diffs/runtime.md`](diffs/runtime.md) (exact `OLD → NEW` for
  `docs/contracts/runtime.md`; §D lists the cross-file requests);
- [`../m3-sample.md`](../m3-sample.md) §§2–3 (run rules, layout, camera);
- [`../m3-acceptance.md`](../m3-acceptance.md) rows B04–B10, B22;
- accepted `runtime.md` §§3/4/5/6/8/12/13/14, `project-model.md` §21,
  `commands.md` §5.4;
- `fixtures/m3/gameplay/**`, `fixtures/m3/camera/**` (this packet's numeric,
  state and camera fixtures + `tools/check-fixtures.mjs`).
...
numbers are never renumbered: new runtime material is appended as §15/§16 by
[`diffs/runtime.md`](diffs/runtime.md).
...
A zone is a `components.gameZone` entity (packet 39 **PROPOSED** `model.md`
§23.3.1): `role` ∈
...
is recorded as a cross-file request in [`diffs/runtime.md`](diffs/runtime.md) §D.
...
Nothing in this packet is a test result: this document and its fixtures are
proposals, and every implementation claim belongs to packets 49–51.

---

## 11. Public exports (proposed, implemented by 49–51)
