...
| 06 | Pure commands and history | B | 05 | review 2026-09-17 (docs/handoffs/06-review.md): **changes still required** — 1 P1 (pipeline order vs §6.1 step 4), 3 non-gating P2s; **repair (round 1) 2026-09-17** (handoff 06 "Repair update"): F1 fixed (envelope → revision check → `revision_exhausted` → args schema → op) + 8 regression tests; reviewer repros A1/A3/A5 re-run green against the real package; 386/386, fixtures 71/71 byte-identical; F2–F4 carried forward (no code changes); **pending re-review**; next: re-review run | docs/handoffs/06.md |
