...
| 7 | Editor layout like Unity: Hierarchy left, Scene/Game tabs centre, Inspector right, bottom dock (Assets, Prefabs, Behaviors, Gameplay, Media, Problems), draggable splitters remembered per browser | — | done 2026-09-23 — Play opens in the Game tab (Scene tab keeps the game running behind the viewport); Assets and Behaviors panels re-laid as wide rows; `tests/e2e/layout.e2e.ts` checks dock positions, tabs, Play in the Game tab, splitter drag + persistence. Owner look pending |
