...
| Default input (move 1D, jump, attack, interact…), HUD prompts naming A/D and Space | input.ts, game-host hud.ts | kept: the built-in controller moves in the X/Y plane; the classic HUD's prompt text does not follow rebinding (a genericity gap, not demo-shaped; left open) |
