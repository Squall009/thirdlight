...
| 15.2 Scene handles, instance copies | done 2026-09-24 | 6a81f23 |
...
| 15.5 defaults audit | done 2026-09-24 | c3c48b2, 1d83692 |
| 15.6 wrap-up | done 2026-09-24 | - |
...
- 2026-09-24 (15.2): one handle system (`editor/src/session/handles.ts`, pure) reads every component handle the descriptors list (`handleShapesOf`: the bound fields → a small model in the handle's frame), gives its grips, drags them (`dragGrip`, snapped, inside the bound fields' ranges) and says which `setComponent` stores it (`commitValue`: the changed top-level fields whole, nothing when unchanged) — one command on release, one undo step. It replaces the phase 14.0 size handles (the capsule outline and "Fit to model" stay in `size-handles.ts`) and the 9.12 waypoint handles (now the `path` handle, grips only for the selection; the other movers keep their dots).
- 2026-09-24 (15.2): the descriptors say what a generic system cannot guess, as handle data: `follows` (a local handle's frame follows the object's position only — areas, paths, ranges, the capsule —, its rotation about Z — a collider —, its rotation — a spot cone, whose direction is in the light's own frame —, or its whole transform — a box mesh's size), `anchor: 'bottom'` (an enemy's body stands on its feet), `along: 'x'` + `band` (an enemy's chase distance and an audio source's range are horizontal distances; the chase band's half height is `chaseHeight`) and `loop` (a mover path closes in loop mode). The directional light's handle stays `world`; the spot cone became `local` (it was listed `world`, but the runtime and the bake turn a spot's direction with its object).
- 2026-09-24 (15.2): grip drags: sizes/radii/ranges/polygon corners snap to 5 cm, path points and world bounds to the 0.25 m translate grid (as the 9.12 waypoints did), a cone's half-angle to 5° (1° steps are too fine to aim with a mouse), directions to 0.05 per component (the field's step; stored normalised then snapped — the field need not be unit length). Size, radius, capsule and polygon grips move on the frame's X/Y plane, a box's depth grip along its Z axis, cone and direction grips on a camera-facing plane. A zero radius (chase 0, an unlimited light) puts its grip 0.25 m out so it is not under the object's own gizmo. Ranges and bounds keep a 5 cm gap between their ends (the validators want left < right). Grips keep a constant screen size.
- 2026-09-24 (15.2): paths and polygons: a small grey grip in the middle of each segment inserts a point there when dragged; Alt+click deletes a point (the list's minimum — 1 waypoint, 3 corners — is refused with a notice). A polygon that stops being convex or counter-clockwise (the collider validator's rules, checked after millimetre rounding) is previewed red and not sent; the notice says why — the command would refuse it anyway. A press on a grip never picks what is under it.
- 2026-09-24 (15.2): camera follow bounds get handles only while they are set (absent means "anywhere"; adding them is the Inspector's "+ add"). The `point` handle kind is implemented and unit-tested through the model, but no descriptor field uses it yet, so it has no e2e.
- 2026-09-24 (15.2): drawn now: an enemy's chase band (x ± chase around it, feet ± chase height — the runtime's test), a selected camera's real frustum (fovY, near, far at the game view's aspect: the Game preview's while it plays, else the window's — an export fills the window; 16:9 before either is known); the small camera marker stays for unselected cameras (a 100 m frustum on every camera would bury the view). As logged in 15.0, v4 has no level bounds or kill height, so plan item "level bounds and killY" has nothing to draw.
- 2026-09-24 (15.2): "Add collider → box / polygon from model outline": the editor reads the vertices of the models it already draws for the object (its own and its children's; at most ~50k per mesh — the hull needs only the extremes) on the play plane in the collider's frame (position + rotation about Z); the polygon is the convex hull reduced to the collider's corner limit (8, from the descriptor) by dropping the corner that loses the least area (the outline stays inside the hull), rounded to millimetres and cleaned to stay valid; the box is the bounds when centred on the origin (within 1 cm), otherwise the same rectangle as a 4-corner polygon with a notice (a box collider has no offset). Offered in "+ Add component", the Component menu's Collider submenu and as buttons in the Collider section (replacing the shape).
- 2026-09-24 (15.2): `playerSpawn.facing` (`none`/`left`/`right`, v4 only, optional — absent and `none` mean the same, `null` removes it; v3 spawns keep no fields): at start, replay and respawn the runtime turns the face-movement models under the player to the spawn's side at once (before the reset's prev := curr, so the first frame shows it). The player itself has no facing state of its own in the engine — face movement is how a model shows direction — so this is the generic meaning; a spawn without a facing changes nothing (every recorded replay is unchanged).
- 2026-09-24 (15.2): `animator.parameters` is edited from the controller's parameter list (float/int numbers, bool checkboxes; triggers start unset and are not listed; values not in the list are shown and removable) — the generic Inspector's map widget for a `keyRef: animatorParameter` map, fed by the controllers in the field context.
- 2026-09-24 (15.2): instance copies: with the set selected, a click on a copy selects it (the raycast's instance id); the gizmo drives a stand-in child placed at the copy's local transform and previews the copy while dragging (`BuiltInstanceSet.setCopy`); release, Del, and a brush stroke (points at least 1 m apart on the surface under the pointer, upright, scale 1) each build the new copy list, publish it through the buffer route `tl_instance_buffer` uses and store it with one `setComponent instances {buffer, count}` — one undo step, the old buffer still stored. A set keeps at least one copy (delete the object instead). MCP gained `tl_instance_buffer {digest}` to read a set's copies (up to 4096, 9 significant digits so an unchanged float32 republishes to the same bytes), so MCP edits copies by the same path.
