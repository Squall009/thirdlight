...
- 2026-09-26 (21.6): the leak test's "editor scene closed/opened" read 20–31 WebGL vertex arrays against 18 in some full runs: 21.5's VAO sweep ran only before the next VAO was created, so the VAOs of attributes destroyed or collected after the last cycle stayed until then (bounded, timing-dependent). A destroyed attribute and a collected one (FinalizationRegistry) now schedule a sweep in their own task; the scenario reads 14 → 14 (unit test for the scheduled sweep).
- 2026-09-26 (21.6): a gate run found a real editor race: an edit of the existing game block (Gameplay tab) made before the previous edit was acked was refused as `revision_conflict` (setGameConfig is a whole-document op, sent with args fixed at call time). The partial patch it sends (changed top-level fields) is now built at send time and rebased over the editor's own acked edits; the inspector e2e holds the first edit's HTTP ack and WS event back to make the case deterministic.
