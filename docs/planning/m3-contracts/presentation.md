# Packet 41 — presentation contract: rendering, animation and audio (PROPOSED)

**PROPOSED — not accepted.** Packet 41 (`docs/planning/m3-packets.md` §41). This
document is the proposed **new contract home** for M3 presentation: the bounded
lighting/shadow profile, the three copied primitive presets, the rigid-node
animation role profile and its atomic reimport, the bounded PCM-WAV audio
profile and its injected browser owner, and the checkpoint activation
appearance. Nothing in `docs/contracts/**` or `packages/**` is changed; Gate K
records accept/reject per diff, and a separate docs-only promotion applies
accepted rows before packet 44.

Read basis (bounded): `AGENTS.md`; `docs/STATUS.md`; `planning/m3-plan.md`
§§2/2.1/3.3/4/5; `planning/m3-packets.md` §41 + "Rules for every packet" +
"Contract drafting and promotion" + packets 47/52/53/54/57; `planning/m3-sample.md`
§§2–4; `planning/m3-acceptance.md` rows B11–B14; `planning/m3-contracts/model.md`
§§23.3.1/23.3.1a/23.3.4/23.3.5/23.3.6/23.3.7/23.4/23.6/23.10;
`planning/m3-contracts/authoring.md` §§A3.3/A3.5/A5/A8;
`planning/m3-contracts/gameplay.md` §6; `planning/m3-contracts/baseline.md` §1;
`handoffs/38.md`; `handoffs/39.md`; `handoffs/40.md`; accepted contracts
`project-model.md` §§18.1/18.5/18.6/18.7/18.9/19/20, `runtime.md` §§6/8/9/12/13,
`commands.md` §§5.4/8.5, `dependencies.md` §§3/4/7; public exports of
`three-adapter` (`.`, `./gltf-loader`) and `asset-pipeline` (`.`);
`m2-contracts/assets.md` §§5–8 and `m2-contracts/delivery.md` §§2/4.4 (house
style).

Companion diffs (all PROPOSED): `diffs/project-model.md` (packet-41 section),
`diffs/runtime.md` (packet-41 section), `diffs/commands.md` (packet-41 section).
Fixtures: `fixtures/m3/media/**`.
...
- v3 scene/storage/catalog data, the `activation` slot's presence/requiredness,
  ...
  (`model.md` §23). This contract supplies exactly the fields 39 marked as
...
The authored values are `components.light` (`model.md` §23.3.4): at most one
...
3. The light entities' own `transform` is irrelevant to the light
   (`model.md` §23.3.4): the adapter reads only the component value.
...
— `baseline.md` §1). The profile is frozen to that recorded, real-browser
...
| WebGL exists but the context is not WebGL 2 | same `render_unsupported`; the M3 target is WebGL 2 (`baseline.md` §1: software WebGL 2) | **hard** (unplayable) |
...
Model-wide, the closed preset table is packet 39's `model.md` §23.3.5. This
...
reference is persisted by applying one (`model.md` §23.3.5). The realized
...
Applying a preset is exactly the accepted command of `authoring.md` §A3.3:
...
- An entity with `model` and no `surface` keeps the GLB's authored materials
  (`model.md` §23.3.5; M2 behavior unchanged). GLB materials are never
...
`(assetId, version)`**: a reimport appends a new version and never moves the
recorded binding (39 §23.6.4). The entity data never carries a bare clip name
...
The container order is packet 39's (`model.md` §23.8 step 3 before ranges).
...
persisted (`model.md` §23.3.5, accepted §18.1).
...
`authoring.md` §A3.5/§A4 — see C41-2/C41-3 in `diffs/commands.md`):
...
3. The appended `AssetVersion` and the replaced `roles` value are applied in
   **one** transaction: one revision, one history entry. The change is
   `{ type: "publishAsset", mode, assetId, previous, next, animation: {
   entityId, previous, next } }`; the inverse restores both the previous record
   and the previous `roles` value (or removes the created record and restores
   the previous binding). `undo`/`redo` move both together or neither.
...
The exact OLD/NEW text is in `diffs/project-model.md` (packet-41 section,
...
**K-2/FU-4**; runtime row `diffs/runtime.md` R40-17):
...
at `AUDIO_MAX_REFERENCED_CUES = 6` (`limits_exceeded` `audio_cues`). The
...
9. **Never a server sound device.** No backend, MCP, export build or Node
   ...
   bytes. A headless container has no audio device (`baseline.md` §1), so
...
decibel/bit-exact playback claim, and no autoplay success. `baseline.md` §1
...
(`model.md` §23.3.1a), required on every `role: "checkpoint"` zone:
...
- **57** authors the value through the existing `setComponent(zoneId,
  ...
  (`authoring.md` §A8 row 11). It shows a preview through the same injected
...
| audio | referenced cues | 6 | `limits_exceeded` (`audio_cues`) |
...
|---|---|---|
| A + B | `commands.md` §5.4 (rows + extended `limits_exceeded` names), `project-model.md` §23.9 mirror | this packet's `diffs/commands.md` section, promoted at K |
| C | `asset-pipeline` `ImportDiagnosticCode`/`ImportLimitName` (packet 47 implements) | this packet's `diffs/commands.md` note; 47 realizes |
...
| Unit | Subpath | Additions (PROPOSED; realized by the named packet) |
|---|---|---|
...
## 41.11 Contract-change requests (numbered; exact destinations)

| # | Destination | Request |
|---|---|---|
| C41-1 | `gameplay.md` §6 (packet 40) | **Resolved** by the Gate K bounded repair (K-2/FU-4): the read-only `GameView.playerMotion: { speed, grounded }` field (§41.3.7) with its exact derivation is now committed in `gameplay.md` §6 and `diffs/runtime.md` R40-17, so the role selector consumes committed motion. Additive; the packet-40 `game-view` fixture is extended by packet 43. |
| C41-2 | `authoring.md` §A3.5/§A4 (packet 39) | `publishAsset` `args` gain the optional `animation: { entityId, roles }` field and the atomic rule of §41.3.4 (required for a reimport of a referenced animated asset). 39's file is not edited here; the exact destination text is in this packet's `diffs/commands.md` section. |
| C41-3 | `commands.md` §8.5 + §5.4 | The atomic reimport change/inverse shape (§41.3.4) and the code/limit rows of §41.7.2. |
| C41-4 | `project-model.md` §18.1 rule 3 | The version-local clip-role exception (PM41-1 in `diffs/project-model.md`) — the only accepted model rule this packet replaces. |
