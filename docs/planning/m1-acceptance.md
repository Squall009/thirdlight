...
| Demo math exact (A=0.5, T=4.0, 120 Hz, `x0 + A·sin(…)`), all boxes, camera static, bounded ±0.5 m (§7) | unit tests at stepIndexes 0, 120 (quarter period: x = x0 + A), 240 (half period: x = x0), 360 (x = x0 − A); a group + camera entity untouched | 08 | test results (positions) |
