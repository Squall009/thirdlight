...
Revision: v0.1 + Gate D bounded follow-up FD-1 (2026-09-18, docs-only): steps
6/7 expected revision corrected r4 → r5 (step 5 of this same table ends at
r5 — the commands.md §9.1 truncation edit — and the packet-13 run is
consistently r5; see docs/handoffs/gate-d.md).
...
| 6 | Close the editor; **restart the backend process**; reopen the editor | re-attach returns the state at `revision: 5` (step 5 ends at r5 — the fresh edit truncates the redo stack) — the durable envelope survived (workspace.md §5 G1/G2); the projection is full-state resync (sessions.md §8) | attach result after restart; the on-disk envelope bytes identical to the pre-restart bytes |
| 7 | Start play (UI Play button) | play session `active → presented` (sessions.md §10.2); preview origin loads; bridge handshake + `tl.snapshot` relay (sessions.md §13.4); the **snapshot revision is displayed** in the play panel and preview HUD (`demo-m1@r5`); the demo box oscillates ±0.5 m (runtime.md §7.1) | screenshots at t≈0 and t≈2 s (mid-oscillation); `play.started`/`play.preview.ready` events; a `queryProject` **during** play shows `revision: 5` and the authored (un-moved) transform — play motion does not change the authoring scene (runtime.md §10) |
...
| Demo math exact (A=0.5, T=4.0, 120 Hz, `x0 + A·sin(…)`), all boxes, camera static, bounded ±0.5 m (§7) | unit tests at stepIndexes 119 (x = x0 + A), 239 (x = x0), 359 (x = x0 − A), 479 (x = x0) — the exact quarter/half/three-quarter/full-period points under the §7.1 `(stepIndex + 1)` offset (SIM_HZ·T = 480 steps per period); a group + camera entity untouched | 08 | test results (positions) |
...
| Preview receives no credentials; the preview bundle has no authoring token/API URL (§2/§13.2) | bundle scan (dependencies.md §5.4 patterns a–j over the preview bundle) + manual inspection of the page config (only `v` + `authoringOrigin`) | 10 | scan output; the config snippet |
...
| Single relative `fetch(./snapshot.json)`; browser-only target (§5.3) | the scan (export.md §5.4 pattern d) + the `node:`/`process.`/`__dirname` scan (e/f) + the no-XHR/WebSocket pattern (j) | 12 | scan output |
| Forbidden-content scan, all 10 patterns, no silent exceptions (§5.4) | the scan on all emitted bytes; a deliberate fixture (a bundle containing the authoring origin) fails with the reported hit | 12 (incl. the negative fixture) | both scan outputs |
