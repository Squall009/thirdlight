...
- 2026-09-24 (14.1): ids are `spawn-<n>`, n counting from 1 in each run (a replay of the same inputs gets the same ids) and skipping any id already in the game; the renderer compares entity objects, not only ids, so a reused id in a later run is a fresh object. Children get their own `spawn-<n>`; behavior values naming a localId of the prefab are remapped to the copy (as `instantiatePrefab` does; any string value equal to one of the prefab's localIds is treated as such a reference, since the runtime has no declarations). Copies carry `prefab {prefabId, localId}` provenance.
...
- 2026-09-24 (14.1): a spawned entity's own script runs, but transform intents still need the entity in the behavior's published `ownedTransforms`, which cannot name runtime ids; moving copies is done with a mover or an enemy component (the e2e projectile is a mover). Dynamic ownership for spawned copies is left for the visual-scripting/ownership work (not done here).
