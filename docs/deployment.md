...
area as a **Graph: <name>** tab next to Scene and Game (× closes it). The
Inspector on the right shows the selected node (its fields), wire (its
type or implicit conversion), group (title, colour) or comment.
